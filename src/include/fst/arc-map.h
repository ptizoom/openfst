...
void ArcMap(MutableFst<A> *fst, C *mapper) {
  using FromArc = A;
  using ToArc = A;
  //PTZ191108 not using StateId = typename FromArc::StateId;
  //PTZ191108 not
...
void ArcMap(const Fst<A> &ifst, MutableFst<B> *ofst, C *mapper) {
  ...
  //PTZ191108 not using Weight = typename FromArc::Weight;
  ofst->DeleteStates();
...
class ArcMapFstImpl : public CacheImpl<B> {
 public:
  ...
  size_t Properties() const override { return Properties(kFstProperties); }

  // Sets error if found, and returns other FST impl properties.
  size_t Properties(size_t mask) const override {
 ...
 private:
  void Init() {
    ...
    if (fst_->Start() == kNoStateId) {
      ...
      size_t props = fst_->Properties(kCopyProperties, false);
...
class FromGallicMapper {
 public:
  ...
  size_t Properties(size_t inprops) const {
    size_t outprops = inprops & kOLabelInvariantProperties &
