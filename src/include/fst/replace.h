...
//
//   // Required constuctor
//   ReplaceStateTable(const std::vector<std::pair<Label, const Fst<A>*>>
//   &fst_tuples,
//                     Label root);
//
 ...
 public:
  ...
  uint64 operator()(const ReplaceStateTuple<S, P>& tuple) const {
    return tuple.prefix_id * (cumulative_size_array_->back()) +
           cumulative_size_array_->at(tuple.fst_id - 1) + tuple.fst_state;
  }

 private:
  const std::vector<uint64>* cumulative_size_array_;
};
...
class ReplaceStackPrefix {
 public:
  ...
  // copy constructor
  ReplaceStackPrefix(const ReplaceStackPrefix& x) : prefix_(x.prefix_) {}
...
class VectorHashReplaceStateTable {
 public:
  ...
  typedef ReplaceStackPrefix<Label, StateId> StackPrefix;
  typedef CompactHashBiTable<PrefixId, StackPrefix,
                             ReplaceStackPrefixHash<Label, StateId>>
      StackPrefixTable;

  VectorHashReplaceStateTable(
      const std::vector<std::pair<Label, const Fst<A>*>>& fst_tuples,
      Label root)
      : root_size_(0) {
    ...
    state_table_ = new StateTable(
        ...
        new ReplaceFingerprint<StateId, P>(&cumulative_size_array_), root_size_,
  ...
  ~VectorHashReplaceStateTable() { delete state_table_; }
 ...
 private:
  StateId root_size_;
  std::vector<uint64> cumulative_size_array_;
...
class DefaultReplaceStateTable
    : public CompactHashStateTable<ReplaceStateTuple<typename A::StateId, P>,
 ...
 public:
  ...
  typedef P PrefixId;
  typedef ReplaceStateTuple<StateId, P> StateTuple;
  typedef CompactHashStateTable<StateTuple, ReplaceHash<StateId, PrefixId>>
      StateTable;
  typedef ReplaceStackPrefix<Label, StateId> StackPrefix;
  typedef CompactHashBiTable<PrefixId, StackPrefix,
                             ReplaceStackPrefixHash<Label, StateId>>
...
class ReplaceFstImpl : public CacheBaseImpl<typename C::State, C> {
 ...
 public:
  ...
  ReplaceFstImpl(const std::vector<std::pair<Label, const Fst<A>*>>& fst_tuples,
    ...
    for (size_t i = 0; i < fst_tuples.size(); ++i) {
      ...
      if (nonterminal >= 0) all_negative = false;
      ...
      if (i) {
        if (!CompatSymbols(InputSymbols(), fst->InputSymbols())) {
          FSTERROR() << "ReplaceFstImpl: Input symbols of Fst " << i
                     << " does not match input symbols of base Fst (0th Fst)";
  ...
  Weight Final(StateId s) {
    if (HasFinal(s)) {
      ...
      if (always_cache_ || HasArcs(s)) SetFinal(s, final);
      return final;
    }
  }
  ...
  size_t NumArcs(StateId s) {
    if (HasArcs(s)) {  // If state cached, use the cached value.
      ...
      if (ComputeFinalArc(tuple, 0)) num_arcs++;
  ...
  bool IsNonTerminal(Label l) const {
    ...
    typename NonTerminalHash::const_iterator it = nonterminal_hash_.find(l);
  ...
  void Expand(StateId s) {
    ...
    ArcIterator<Fst<A>> aiter(*(fst_array_[tuple.fst_id]), tuple.fst_state);
    Arc arc;
  ...
  bool ComputeFinalArc(const StateTuple& tuple, A* arcp,
    ...
    if (fst->Final(fst_state) != Weight::Zero() && tuple.prefix_id) {
      if (arcp) {
        ...
        if (flags & kArcWeightValue) arcp->weight = fst->Final(fst_state);
      }
      return true;
    } else {
      return false;
  ...
  T* GetStateTable() const { return state_table_; }

  const Fst<A>* GetFst(Label fst_id) const { return fst_array_[fst_id]; }
 ...
 private:
  ...
  std::vector<const Fst<A>*> fst_array_;
  Label root_;
...
//
// Note that the constructor uses a vector of std::pair<>. These correspond
...
class ReplaceFst : public ImplToFst<ReplaceFstImpl<A, T, C>> {
 public:
  ...
  ReplaceFst(const std::vector<std::pair<Label, const Fst<A>*>>& fst_array,
             Label root)
      : ImplToFst<Impl>(std::make_shared<Impl>(
            fst_array, ReplaceFstOptions<A, T, C>(root))) {}
...
class StateIterator<ReplaceFst<A, T, C>>
 ...
 public:
  explicit StateIterator(const ReplaceFst<A, T, C>& fst)
      : CacheStateIterator<ReplaceFst<A, T, C>>(fst, fst.GetMutableImpl()) {}
 ...
 public:
  ...
  void SetFlags(uint32 f, uint32 mask) {
    ...
    if (!(flags_ & kArcNoCache) && data_flags_ != kArcValueFlags) {
      if (!fst_.GetImpl()->HasArcs(state_)) data_flags_ = 0;
    }
    ...
    if ((f & kArcNoCache) && (!data_flags_)) Init();
  }
...
class ReplaceFstMatcher : public MatcherBase<A> {
 public:
  ...
  void InitMatchers() {
    const std::vector<const Fst<A>*>& fst_array = impl_->fst_array_;
  ...
  ReplaceFstMatcher<A, T, C>* Copy(bool safe = false) const override {
    return new ReplaceFstMatcher<A, T, C>(*this, safe);
  }

  ~ReplaceFstMatcher() override {
    for (size_t i = 0; i < matcher_.size(); ++i) delete matcher_[i];
  }
 ...
 private:
  ...
  std::vector<LocalMatcher*> matcher_;
...
template <class A, class T, class C>
inline void ReplaceFst<A, T, C>::InitStateIterator(
...
// input argument is a vector of std::pair<>. These correspond to the tuple
