...
for NAME in $@ ; do
  ...
  __NEWARGLINE="$__NEWARGLINE $NAME"
done
...
echo
if [ -z "$__NEWARGLINE" ]; then
  echo "I am going to run ./configure with no arguments - if you wish "
  echo "to pass any to it, please specify them on the $0 command line."
else
  echo "using: $__NEWARGLINE"
fi
echo

"$SRCDIR/configure" $__NEWARGLINE || {
  echo
